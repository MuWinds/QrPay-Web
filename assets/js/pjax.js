...
(function(h) {
	function l(H, I, J) {
		...
		return this.on("click.pjax", H, function(M) {
			...
			if (!L.container) {
				L.container = h(this).attr("data-pjax") || K
			}
			m(M, L)
		})
	}
	function m(M, I, J) {
		...
		if (M.which > 1 || M.metaKey || M.ctrlKey || M.shiftKey || M.altKey) {
			return
		}
		if (location.protocol !== L.protocol || location.hostname !== L.hostname) {
			return
		}
		if (L.href.indexOf("#") > -1 && z(L) == z(location)) {
			return
		}
	...
	function C(H) {
		...
		H.beforeSend = function(Q, P) {
			...
			if (!K("pjax:beforeSend", [Q, P])) {
				return false
			}
			if (P.timeout > 0) {
				J = setTimeout(function() {
					if (K("pjax:timeout", [Q, H])) {
						Q.abort("timeout")
					}
				}, P.timeout);
				P.timeout = 0
		...
		if (I && I.container) {
			...
			if (K.length) {
				...
				if (M) {
					...
					var L = h.Event("pjax:beforeReplace", {
						state: I,
						previousState: O
					});
					K.trigger(L, [M, Q]);
	...
	function y(L, N, P) {
		...
		if (P.fragment) {
			if (P.fragment === "body") {
				...
				var O = o(J, P.fragment).first()
			}
			if (O.length) {
				K.contents = P.fragment === "body" ? O : O.contents();
				if (!K.title) {
					K.title = O.attr("title") || O.data("title")
				}
			}
		} else {
			if (!H) {
