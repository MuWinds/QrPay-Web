...
case 'resetKey':
	if(isset($_POST['submit'])){
		$key = random(18);
        $key1=authcode($key, 'base64_decode', $conf['KEY']);
		$sql = "UPDATE pay_user SET `key`='$key1' WHERE pid='$pid'";
		if($DB->exec($sql)!==false)exit('{"code":0,"msg":"重置密钥成功","key":"'.$key1.'"}');
		else exit('{"code":-1,"msg":"重置密钥失败['.$DB->error().']"}');
	}
break;
case 'edit_pwd':
	    $userrow=$DB->query("SELECT * FROM pay_user WHERE pid='{$pid}' limit 1")->fetch();
        $oldpwd = daddslashes(strip_tags($_POST['oldpwd']));
        $newpwd = daddslashes(strip_tags($_POST['newpwd']));
        $newpwd2 = daddslashes(strip_tags($_POST['newpwd2']));
        	if(!empty($userrow['pwd']) && $oldpwd==null || $newpwd==null || $newpwd2==null){
		exit('{"code":-1,"msg":"请确保每项都不为空"}');
        } else if ($oldpwd != $userrow['pwd']) {
	...
	}elseif ($newpwd == $userrow['qq']) {
		exit('{"code":-1,"msg":"新密码不能和qq相同"}');
	}elseif (is_numeric($newpwd)) {
		exit('{"code":-1,"msg":"新密码不能为纯数字"}');
	}
	$pwd = daddslashes($newpwd, $pid);
    $sqs=$DB->exec("update `pay_user` set `pwd` ='{$pwd}' where `pid`='$pid'");
	if($sqs!==false){
...
case 'edit_codename':
	...
	$sqs=$DB->exec("update `pay_user` set `codename` ='{$codename}' where `pid`='$pid'");
	if($sqs!==false){
