...
if($mod=='pay_n'){
	$userrow=$DB->query("SELECT * FROM pay_user WHERE pid='{$pid}' limit 1")->fetch();
  $qqcookie=$_POST['qqcookie']?daddslashes($_POST['qqcookie']):请输入cookie;
  $payQQ=$_POST['payQQ']?daddslashes($_POST['payQQ']):请输入QQ;
	if($qqcookie==null){
		echo'<script>alert("请确保cookies不能为空！");location.href="./qqcookie.php"</script>';
	}else{
		$sds=$DB->query("update pay_user set qqcookie='$qqcookie'  where pid='{$pid}'");
		$sdsa=$DB->query("update pay_user set payQQ='$payQQ'  where pid='{$pid}'");
		if($sds)echo'<script>alert("修改成功");location.href="./qqcookie.php"</script>';
		else echo'<script>alert("修改失败");location.href="./qqcookie.php"</script>';
	}
}
...
$zt = file_get_contents("http://".$_SERVER['HTTP_HOST']."/corn/qqpaypay.php?id=$pid");
if ($zt == "-1") {
    $ztr = "X异常";
...
<div class="content-page">
 <div class="panel panel-default">
  ...
  <div class="panel-body">
			<form class="form-horizontal devform" action="./qqcookie.php?mod=pay_n" method="post">
			<div class="form-group">
			    <div class="layui-card-header">QQ免挂</div>
         ...
         点击获取登录 扫码登录后复制填写到余额栏
          <small style="color: red">(QQ钱包无余额会显示异常)</small>  <small style="margin-top: 1px;cursor: pointer" id="1checkOrder" class="badge badge-info">点击获取登录</small>
					...
					<div class="col-sm-9">
						<input class="form-control" type="text" name="qqcookie" id="qqcookie" value="<?php echo $zt;?>">
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<input class="form-control" type="text" name="payQQ" id="payQQ" value="<?php echo $userrow['payQQ']?>">
