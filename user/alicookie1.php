...
if(!checkIfActive('index,userinfo,') AND !$userrow['zfb']){
...
if($mod=='pay_n'){
	$userrow=$DB->query("SELECT * FROM pay_user WHERE pid='{$pid}' limit 1")->fetch();
  $alicookie1=$_POST['alicookie1']?daddslashes($_POST['alicookie1']):请输入授权值;
	if($alicookie1==null){
		echo'<script>alert("请确保cookies不能为空！");location.href="./alicookie1.php"</script>';
	}else{
		$sds=$DB->query("update pay_user set alicookie1='$alicookie1'  where pid='{$pid}'");
		if($sds)echo'<script>alert("修改成功");location.href="./alicookie1.php"</script>';
		else echo'<script>alert("修改失败");location.href="./alicookie1.php"</script>';
	}
}


$zt = file_get_contents("http://".$_SERVER['HTTP_HOST']."/corn/alipaypay1.php?id=$pid");
if ($zt == "-1") {
    $ztr = "X异常";
...
<div class="content-page">
 <div class="panel panel-default">
  ...
  <div class="panel-body">
			<form class="form-horizontal devform" action="./alicookie1.php?mod=pay_n" method="post">
			
			<div class="form-group">
			     <div class="layui-card-header">支付宝免挂</div>
			    <div class="layui-card-body">
         扫码登录后提交
          <small style="color: red">(显示余额为监控成功)</small><a href="/user/2alicookie.php" class="badge badge-info" style="margin-top: 1px;cursor: pointer" target="_blank">点击手动填cookie</a></small>
          </div>
					<label class="col-sm-2 control-label">监控余额</label>
					<div class="col-sm-9">
						<input class="form-control" type="text" name="dqalicookie1" id="dqalicookie1" value="<?php echo $zt;?>" disabled>
					</div>
				</div>
...
function GET_QRlist() {
	...
	if(Update_QrUrl==1 && Update_Get_Cookie==0){
		$.get("../qrcode/cookie/alipay/Ajax.php?act=Get_Cookie",{id:QrCode_id},function(data){
			if(data.cookie!=''){
				Update_Get_Cookie=1; 
				$("#LoginQrcode").html('<textarea class="form-control" name="alicookie1" id="alicookie1" rows="10">'+data.cookie+'</textarea>');//输出登录二维码
...
window.setInterval(function () {
  ...
  <h9 class="list-group-item"><span class="pull-right"> </span><em class="fa fa-fw fa-volume-up mr"></em>
				<td align="center">如果登录提交后还是失效，请重新扫码登录<br></h9>
  <h9 class="list-group-item"><span class="pull-right"> </span><em class="fa fa-fw fa-volume-up mr"></em>
