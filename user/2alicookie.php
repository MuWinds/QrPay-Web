...
if($mod=='pay_n'){
 $userrow=$DB->query("SELECT * FROM pay_user WHERE pid='{$pid}' limit 1")->fetch();
  $alicookie1=$_POST['alicookie1']?daddslashes($_POST['alicookie1']):请输入cookie;
 if($alicookie1==null){
  echo'<script>alert("请确保cookies不能为空！");location.href="./2alicookie.php"</script>';
 }else{
  $sds=$DB->query("update pay_user set alicookie1='$alicookie1'  where pid='{$pid}'");
  if($sds)echo'<script>alert("修改成功");location.href="./2alicookie.php"</script>';
  else echo'<script>alert("修改失败");location.href="./2alicookie.php"</script>';
 }
}
...
<div class="bg-light lter b-b wrapper-md hidden-print">
  <h1 class="m-n font-thin h3">支付宝轮训①通道设置</h1>
</div>
<div class="wrapper-md control">
 <div class="panel panel-default">
  ...
  <div class="panel-body">
   <form class="form-horizontal devform" action="./2alicookie.php?mod=pay_n" method="post">
   
   <div class="form-group">
     <label class="col-sm-2 control-label">支付宝cookies配置</label>
     <div class="col-sm-9">
      <input class="form-control" type="text" name="alicookie1" id="alicookie1" value="<?php echo $userrow['alicookie1']?>">
