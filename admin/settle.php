...
<?php
	$list=$DB->query("SELECT * FROM pay_user WHERE money>='10'")->fetchAll();
...
<div class="panel panel-primary">
	<table class="table table-bordered table-condensed">
		<tbody>
			...
			foreach($list as $row) {
			echo '<tr>
			<td uin="'.$row['pid'].'"><input name="uins" type="checkbox" id="uins" class="uins" value="'.$row['pid'].'" '.($row['transfer_status']!=1?'checked':null).'>'.$row['pid'].'</td>
			<td>'.$row['account'].'</td>
			<td>'.$row['username'].'</td>
			<td class="money">'.$row['money'].'</td>
			<td id="id'.$row['pid'].'" uin="'.$row['pid'].'" class="nocheck recheck" align="center">'.($row['transfer_status']!=1?'<span class="btn btn-xs btn-block btn-primary">立即转账</span>':'<font color="green">已完成</font>').'</td></tr>
			<tr><td><span style="color:silver;">结果</span></td>
			<td colspan="5" id="res'.$row['pid'].'"><font color="blue">'.($row['transfer_status']==1?'支付宝转账单据号:'.$row['transfer_result'].' 支付时间:'.$row['transfer_date']:$row['transfer_result']).'</font></td></tr>';
			}
			?>
		</tbody>
	</table>
