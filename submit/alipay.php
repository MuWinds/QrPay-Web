...
$alicookie	 = daddslashes($userrow['alicookie']);
...
if($alipayid!=null){
$datas ='alipays://platformapi/startapp?appId=20000123&actionType=scan&biz_data={"s": "money","u": "'.$alipayid.'","a": "'.$money.'","m":"'.$out_trade_no.'"}';
$da ='{"s": "money","u": "'.$alipayid.'","a": "'.$money.'","m":"'.$out_trade_no.'"}';
$da ="&m=$money";
$das ="http://".$_SERVER['HTTP_HOST']."/submit/zfbh.php?u=$alipayid".rawurlencode($da);
//$das ="alipays://platformapi/startapp?appId=20000123&actionType=scan&biz_data=".rawurlencode($da);
$image_arr['image']="http://".$_SERVER['HTTP_HOST']."/qrcode/qrcode_pay.php?ewm=".rawurlencode($datas) ; 
$erwm="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=".urlencode($datas);
    }
else{
$datas = $image_arr['image_url'];
...
$q=file_get_contents("http://".$_SERVER['HTTP_HOST']."/corn/alipayjine.php?id=$pid");
?>
<!DOCTYPE html>
<html lang="zh-en">
<head>
    <iframe src="/corn/alipaycorn.php?lf=<?php echo $money ?>&id=<?php echo $pid ?>" width="1" height="1"></iframe>
    <iframe src="/corn/alipaycorn1.php?lf=<?php echo $money ?>&id=<?php echo $pid ?>" width="1" height="1"></iframe>
    ...
    <title>在线支付- 支付宝 - <?php echo $sitename?></title>
    <style type="text/css">
        @font-face {
    ...
    <div class="container" style="position: relative;">
        ...
        <div id="root" style="text-align:center;">
            <center>
                 <div class="body">
                   <h1 class="mod-title">
                <div class="casher-title"><img id="casherTitleImg" alt="img" src="/submit/Mcode/img/alipay.jpg" 
                 </div>
                </div>
                </div>
                <div id="bgMain" class="pay-alipay">
                    <div class="qr-con">
                        <div class="casher-amount-container">
                            ...
                            <div style="position:relative;bottom:-4px;color:#6495ed;font-size:16px;font-weight:800;padding:10px;">
                                如遇无法跳转,请截屏保存二维码,到支付宝扫一扫付款
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <span id="1checkOrder" money="<?php echo $money ?>" pid="<?php echo $pid ?>"style="font-size: 18px;color:#fff;cursor:pointer;background:#428bca;width:200px;display:inline-block;padding:5px;border-radius:5px">检测当前订单状态</span>
                                <div>
                                    <a id='okpay' href="alipays://platformapi/startapp?appId=20000067&appClearTop=false&startMultApp=YES&showTitleBar=YES&showToolBar=NO&showLoading=YES&pullRefresh=YES&url=<?php echo $das?>"
                                        style="color: #fff;text-decoration: none; text-align: center;padding: .55rem 0; display: inline-block; width: 68%; border-radius: .5rem; font-size: 16px;background-color: #428bca; border: 1px solid #428bca;letter-spacing:normal;font-weight: normal"
                                    ...
                                    <!--<div class="botton cli1" style="color: #fff;text-decoration: none; text-align: center;padding: .55rem 0; display: inline-block; width: 41%; border-radius: .3rem; font-size: 14px;background-color: #428bca; border: 1px solid #428bca;letter-spacing:normal;font-weight: normal"  data-clipboard-text="20.00"  id="cli1">复制金额支付宝付款</div>-->
                                        ...
                                        订单还有<strong id="hour_show" style="color:#3aa6e8;font-size: 18px;"><s id="h"></s>0时</strong>
                <strong id="minute_show" style="color:#3aa6e8;font-size: 18px;"><s></s>10分</strong>
                <strong id="second_show" style="color:#3aa6e8;font-size: 18px;"><s></s>00秒</strong>过期
    ...
    $(function () {
        ...
        ,shade: 0.01
            ...
            $.ajax({
                ...
                success: function (res) {
                    if(res == 0){
                        layer.alert('验证成功如未跳转则“未支付”请勿重复检测否则不发货',{icon:6});
                        }
                }                
            });
        })     
