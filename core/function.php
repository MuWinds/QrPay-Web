...
{
	global $conf;
	$url = "http://api.jisuapi.com/qrcode/read?appkey=e3fd7b3e955b65e3&qrcode=".$qrcode;
	$result=get_curl($url);
	$jsonarr = json_decode($result, true);
    return trimall($jsonarr['result']);
}
...
function transferToAlipay($trade_no, $username, $account, $money,$bz){
	global $conf,$DB,$date;

	require SYSTEM_ROOT."f2fpay/lib/AopClient.php";
	require SYSTEM_ROOT."f2fpay/model/request/AlipayFundTransToaccountTransferRequest.php";

$BizContent = array(
	'out_biz_no' => $trade_no, //商户转账唯一订单号
...
$resultCode = $result->$responseNode->code;
	if(!empty($resultCode)&&$resultCode == 10000){
		...
		$DB->exec("update `tx_order` set `transfer_status`='1',`transfer_result`='".$data['result']."',`transfer_date`='{$date}' where trade_no='{$trade_no}' limit 1");
		return json_encode($data);
	}elseif($resultCode == 40004) {
		$data['code']=-1;
		$data['ret']=0;
		$data['msg']='失败 ['.$result->$responseNode->sub_code.']'.$result->$responseNode->sub_msg;
		$data['result']='失败 ['.$result->$responseNode->sub_code.']'.$result->$responseNode->sub_msg;
		$DB->exec("update `tx_order` set `transfer_status`='2',`transfer_result`='".$data['result']."',`transfer_date`='{$date}' where trade_no='{$trade_no}' limit 1");
		return json_encode($data);
	}
}
?>